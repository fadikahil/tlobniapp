...
class ServiceReview extends Model
{
    ...
    {
        return $this->belongsTo(Item::class, 'service_id')->withTrashed();
    }
    ...
    public function scopeSort($query, $column, $order)
    {
        if ($column == "service_name") {
            $query->leftJoin('items', 'items.id', '=', 'service_reviews.service_id')
                ->orderBy('items.name', $order);
        } else if ($column == "user_name") {
            $query->leftJoin('users', 'users.id', '=', 'service_reviews.user_id')
                ->orderBy('users.name', $order);
        } else if ($column == "reviewer_name") {
            $query->leftJoin('users as reviewers', 'reviewers.id', '=', 'service_reviews.reviewer_id')
                ->orderBy('reviewers.name', $order);
        } else {
            $query->orderBy($column, $order);
        }
        return $query->select('service_reviews.*');
    }
}
